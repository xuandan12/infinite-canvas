...
+ [修复] 媒体清理改为仅回收本次删除且全站未引用的候选文件，并保护生图、视频历史与生成中任务，避免刷新或切页后历史结果破图。
+ [修复] 生图与视频工作台在请求发出后立即保存并展示“生成中”记录，跨页面返回时保持任务进度，并原位更新最终结果。
+ [优化] 生图与视频工作台支持在后台任务生成期间新建并提交其他任务，分别记录进度与结果，且防止快速双击重复提交。
+ [优化] 视频创作台与画布的视频比例选项统一以 `16:9`、`9:16`、`1:1` 等标准比例为主展示，并保留横竖屏场景说明。
+ [修复] Grok 视频经 New API 中转时改用通用字符串图片与 metadata 参数协议，并支持最多 7 张参考图，交由服务端 xAI 任务适配器转换和轮询。
+ [修复] Grok Imagine Video 改用 xAI 官方 JSON 异步协议，新增官方 xAI 同源受限代理、视频模型发现与模型专属参数布局，解决 multipart 请求导致的任务创建失败。
+ [修复] 画布视频节点与生成配置节点现与视频创作台统一按当前模型切换 Seedance 专属参数布局，并同步实际生成请求的模型判断。
+ [修复] 官方火山方舟 Seedance 2.0 系列新增同源受限代理，同时兼容标准 `/api/v3` 与 Agent Plan `/api/plan/v3`，解决浏览器直连时的 CORS 失败并保留套餐渠道语义。
+ [修复] Seedance Fast 的 `480p` 请求保持原值发送；结果页不再在元数据读取失败时伪报 `1280x720`，并分离视频时长与生成耗时。
+ [调整] 官方 Ark 渠道改为“添加预置模型”，明确当前 Bearer API Key 无法自动读取已开通模型，最终权限以套餐和实际调用结果为准。
+ [调整] 暂时隐藏全站外部文档与 GitHub 项目入口，等待 Sun canvas 自有地址上线后再恢复。
+ [新增] 首次访问新增 Sun API 一键接入向导，支持验证密钥、读取模型并自动写入工作台配置。
