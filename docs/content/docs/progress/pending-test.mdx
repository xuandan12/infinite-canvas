...
- Grok Imagine Video 接入：Sun API / New API 灰度渠道已完成端到端验证，`grok-imagine-video` 的 720p 文生视频与 `grok-imagine-video-1.5-preview` 的单图 1080p 图生视频均能创建、轮询、播放和下载。官方 xAI 渠道仍需用官方 Key 验证 `/api/xai` 同源代理、模型发现、JSON 创建及 `request_id` 轮询。
- Grok Imagine Video 参数一致性：视频创作台已验证基础模型与 1.5 Preview 的主要文生/图生路径；仍需在空视频节点与生成配置节点切换 Grok/Seedance/通用模型，覆盖 7 种比例、`480p`、1/15 秒、多图参考最长 10 秒、最多 7 张参考图、拒绝参考视频/音频和手机原图自动压缩。
- Seedance 2.0 官方接入：标准 `/api/v3` 与 Agent Plan `/api/plan/v3` 按渠道保留，Vite、Vercel、Docker / Render 通过固定同源 `/api/ark` 路由创建和查询任务；已用无效 Key 验证两条本地代理可到达 Ark，仍需在部署预览用有效套餐 Key 验证创建与轮询、主模型 / Fast / Mini 权限、Fast `480p` 请求与实际输出元数据，以及 GitHub Pages 给出缺少代理的明确提示。官方 Ark 只添加预置候选，不宣称能通过当前 Bearer Key 自动读取已开通模型。
- 全站外部链接入口：暂时隐藏桌面顶部栏、移动端菜单和画布菜单中的文档链接，并隐藏各页面共用状态栏中的 GitHub 项目入口；需确认其他导航间距正常，且提示词参考、版本信息等非外部跳转功能仍可使用。
- Sun API 一键接入：首次访问且尚未完成 AI 配置时自动弹出两步向导，固定使用 `https://newapi.proapihub.com`，验证 API Key 后读取并选择模型；需验证首次显示、关闭后不重弹、连接错误、模型搜索/筛选、配置原子保存、旧未完成渠道保留、默认模型映射和桌面/移动端浅深色表现。
- Sun canvas 首页与品牌：默认页改为暖色双栏创作入口、快速入口和响应式灵感图库，并替换顶部 Logo、站点标题、favicon、默认画布命名与 fork 链接；需验证桌面/移动端、浅色/深色、PNG Logo、`© 2026 Sun canvas` 版权信息、五张有效封面自动补位、图片预览和主要跳转。
